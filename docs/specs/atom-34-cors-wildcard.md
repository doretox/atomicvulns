...
- **SEM SAÍDA B (declarar explicitamente).** Refletir-a-origem + `Allow-Credentials: true` é o **próprio antipadrão ingênuo** de "fazer o CORS funcionar logo, sem pensar" (a receita de copiar-colar de StackOverflow pra "resolver o erro de CORS"). **Diferente do `csrf-basic` (23)** (que É Saída B — o default `SameSite=Lax` do browser resiste ao CSRF ingênuo, exigindo modelar as condições reais), **aqui não há essa ruga**: a reflexão + credenciais é **diretamente** o antipadrão — como o `AES.MODE_ECB` no 32 e o `pickle.loads` no 20. **NÃO inventar uma Saída B.** A única falha é a política. *(Ressalva a confirmar na Fase 2: a **plumbing de origens** — Item 1 — exige cuidado real, como no 23; mas isso é plumbing de lab, não uma "ferramenta que resiste ao antipadrão".)*
