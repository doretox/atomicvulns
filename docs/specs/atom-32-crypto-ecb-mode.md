...
- **ZERO referência pra frente.** **PROIBIDO** citar/antecipar **qualquer** átomo/categoria/variante/técnica futura por número, nome, slug **OU** descrição — inclusive outros A02 futuros, "próxima fase", "outras faces do modo inseguro" modeladas como átomo futuro, ou a posição/ordinal/release de fase. **ESPECIFICAMENTE PROIBIDO:** **bit-flipping** e **padding oracle** (o parêntese do ROADMAP) — são ataques de **outro modo (CBC)**, fora do escopo deste átomo ECB; **não entram** no conteúdo. Quando a tentação surgir, generalizar ("outros modos têm suas próprias falhas") **sem** nomear técnica/modo/átomo.
...
| Escopo (vs parêntese do ROADMAP) | **SÓ ataques genuínos de ECB** (determinismo → penguin + cut-and-paste) | "bit-flipping/padding oracle" são de **CBC** — fora do escopo, **não** mencionar (foreshadow). |
...
| Foreshadow | **ZERO pra frente** (inclusive na spec pública); **sem** bit-flipping/padding oracle | `CLAUDE.md` §5. Não nomear próximos átomos/posição/release/técnicas de outro modo. |
