...
A minimal lab for a mistake that is everywhere in APIs: a **CORS** policy that hands the response to *any* origin that asks, with the victim's cookies attached. The victim is a tiny API — a small HTML portal to log in from, plus JSON endpoints `/login` and `/account` — where `GET /account` returns your private account data as long as your browser holds the session cookie. That endpoint is correct: it requires the cookie and returns the data to the person it belongs to. The flaw is a response *policy* wrapped around it.

To see why that matters you need one browser rule. An **origin** is the triple scheme + host + port (`http://api.lab.localhost:8034`). The **Same-Origin Policy (SOP)** is the rule that lets a script on one origin *send* a request to another origin but forbids it from *reading* the response — it is what stops a script on `evil.com` from calling `bank.com` (where you are logged in) and reading your account page. **CORS (Cross-Origin Resource Sharing)** is how a server can relax that on purpose, by response headers: `Access-Control-Allow-Origin` names which origin may read the response, and `Access-Control-Allow-Credentials: true` adds "…including when the request carried the victim's cookies." These headers are an *instruction to the browser*, not a lock on the server — the server always returns the body; CORS only tells the browser whether to hand that body to the calling script.

This victim gets the policy wrong in the most common way: it reads the request's `Origin` header and **reflects it straight back** into `Access-Control-Allow-Origin`, together with `Access-Control-Allow-Credentials: true`. So it answers "yes, you may read this, with the victim's cookies" to *whatever origin asks* — including a **hostile sibling subdomain** (`evil.lab.localhost`): a compromised, forgotten, or taken-over subdomain of the victim's own site. A victim who is logged in and then visits that sibling's page has their browser make a **credentialed** cross-origin `fetch` (`credentials: 'include'`) to the victim API at `api.lab.localhost`; the browser attaches the victim's cookie, the server replies with the reflected policy, and the browser hands the private response to the attacker's script. Cross-origin data theft, and the attacker never knew the password or touched the cookie.
...
Two origins, three services: the **victim** (a `vulnerable` and a `fixed` copy) and the **attacker** page. They never talk to each other on the backend — the victim's browser makes every cross-origin request — so there is no internal network, no `depends_on`, and no healthcheck. The two are **sibling subdomains of one site**: the victim is `api.lab.localhost` (`:8034` vulnerable, `:8134` fixed) and the attacker is `evil.lab.localhost` (`:8234`). That framing is deliberate. `api.*` and `evil.*` are a **different origin** (so CORS still governs the read) but the **same site** (both under `lab.localhost`), and that matters because modern browsers now **partition third-party cookies by top-level site** — Firefox's Total Cookie Protection is on by default, and third-party cookies are going away elsewhere — so a *purely* cross-site credentialed read (`evil.com` → `bank.com`) no longer carries the victim's cookie at all. Keeping the attacker a *sibling* keeps the cookie flowing, so the **CORS policy is the only control in play** and the exploit reproduces deterministically in today's browsers (Firefox included). All names resolve to `127.0.0.1` (browsers map `*.localhost` to loopback), so the Docker bindings stay on `127.0.0.1` — local-only. The session cookie is `SameSite=None; Secure` (`Secure` works over plain HTTP because `*.localhost` is a *secure context*); `vulnerable` and `fixed` share the `api.lab.localhost` host, so they use distinct cookie names (`session_vuln` / `session_fixed`) to keep their logins apart, since cookies ignore the port — plumbing, not a security difference.
...
- Vulnerable victim: `http://api.lab.localhost:8034` (bound to `127.0.0.1:8034`)
- Fixed victim: `http://api.lab.localhost:8134` (bound to `127.0.0.1:8134`)
- Attacker page: `http://evil.lab.localhost:8234` (bound to `127.0.0.1:8234`)

Use the `*.lab.localhost` hostnames in the browser (not the bare `127.0.0.1` ports) — the distinct hostnames are what make the attacker a cross-origin sibling of the victim. Stop with `./atom down cors-wildcard`. If you prefer raw Docker: `cd atoms/A05-security-misconfiguration/cors-wildcard && docker compose up --build`.
...
2. [`DIFF.md`](./DIFF.md) — the one-policy diff between `vulnerable/` and `fixed/`, and why "trust our own subdomain family," "just drop credentials," and "use `*`" are not the fix.

## Fixed version

The patched API on port 8134 replaces origin reflection with an **exact-match allowlist**: it echoes `Access-Control-Allow-Origin` only when the request's `Origin` is exactly one of the origins it trusts (`partner.lab.localhost`, a stand-in legitimate sibling). An untrusted origin — the hostile sibling `evil.lab.localhost` — gets no CORS header at all, so the browser blocks the cross-origin read and the attacker's script sees a CORS error, not the data. The lesson sharpens here: **not every sibling subdomain is trustworthy — you name the exact ones, you do not trust the whole family.** `GET /account` still returns the same data to the logged-in victim, and the request still reaches the server with the cookie attached — the browser simply refuses to hand the response to a script from an origin the server did not allow. See [`DIFF.md`](./DIFF.md) for why the fix is an allowlist rather than reflecting a "specific" origin, dropping credentials, or the wildcard.
