...
Um laboratório mínimo pra um erro que está em todo canto de API: uma política de **CORS** que entrega a resposta pra *qualquer* origem que perguntar, com os cookies da vítima anexados. A vítima é uma API minúscula — um portal HTML pequeno pra logar, mais os endpoints JSON `/login` e `/account` — onde `GET /account` devolve os dados privados da sua conta enquanto o navegador tiver o cookie de sessão. Esse endpoint está **correto**: ele exige o cookie e devolve o dado a quem é dono dele. A falha é uma *política* de resposta em volta dele.

Pra ver por que isso importa, você precisa de uma regra do navegador. Uma **origem** (origin) é a tripla esquema + host + porta (`http://api.lab.localhost:8034`). A **Same-Origin Policy (SOP)** é a regra que deixa um script de uma origem *disparar* uma requisição a outra origem, mas o **proíbe de LER** a resposta — é o que impede um script de `evil.com` de chamar `bank.com` (onde você está logado) e ler a página da sua conta. **CORS (Cross-Origin Resource Sharing)** é como um servidor pode relaxar isso de propósito, por cabeçalhos de resposta: `Access-Control-Allow-Origin` nomeia qual origem pode ler a resposta, e `Access-Control-Allow-Credentials: true` acrescenta "…inclusive quando a requisição levou os cookies da vítima". Esses cabeçalhos são uma *instrução ao navegador*, não uma tranca no servidor — o servidor sempre devolve o corpo; o CORS só diz ao navegador se ele pode entregar esse corpo ao script que fez a chamada.

Esta vítima erra a política do jeito mais comum: ela lê o cabeçalho `Origin` do request e o **reflete de volta** em `Access-Control-Allow-Origin`, junto com `Access-Control-Allow-Credentials: true`. Ou seja, responde "sim, você pode ler isto, com os cookies da vítima" pra *qualquer origem que perguntar* — inclusive um **subdomínio irmão hostil** (`evil.lab.localhost`): um subdomínio comprometido, esquecido, ou vítima de takeover do próprio site da vítima. Uma vítima que está logada e então visita a página desse irmão tem o navegador fazendo um `fetch` cross-origin **credenciado** (`credentials: 'include'`) pra API-vítima em `api.lab.localhost`; o navegador anexa o cookie da vítima, o servidor responde com a política refletida, e o navegador entrega a resposta privada ao script do atacante. Roubo de dados cross-origin — e o atacante nunca soube a senha nem tocou no cookie.
...
Duas origens, três serviços: a **vítima** (uma cópia `vulnerable` e uma `fixed`) e a página do **atacante**. Elas nunca conversam entre si no backend — o navegador da vítima faz toda requisição cross-origin — então não há rede interna, nem `depends_on`, nem healthcheck. As duas são **subdomínios irmãos de um mesmo site**: a vítima é `api.lab.localhost` (`:8034` vulnerable, `:8134` fixed) e o atacante é `evil.lab.localhost` (`:8234`). Esse enquadramento é proposital. `api.*` e `evil.*` são uma **origem diferente** (então o CORS ainda decide a leitura) mas o **mesmo site** (ambos sob `lab.localhost`), e isso importa porque os navegadores atuais agora **particionam cookies de 3ª parte por site de topo** — a Total Cookie Protection do Firefox é padrão, e os cookies de 3ª parte estão acabando no resto —, então uma leitura credenciada *puramente* cross-site (`evil.com` → `bank.com`) já **não leva o cookie da vítima**. Manter o atacante como *irmão* mantém o cookie viajando, então a **política de CORS é o único controle em jogo** e o exploit reproduz de forma determinística nos navegadores de hoje (Firefox incluído). Todos os nomes resolvem pra `127.0.0.1` (os navegadores mapeiam `*.localhost` pra loopback), então os bindings do Docker ficam em `127.0.0.1` — local-only. O cookie de sessão é `SameSite=None; Secure` (`Secure` funciona sobre HTTP puro porque `*.localhost` é um *secure context*); `vulnerable` e `fixed` dividem o host `api.lab.localhost`, então usam nomes de cookie distintos (`session_vuln` / `session_fixed`) pra manter os logins separados, já que cookie ignora a porta — plumbing, não uma diferença de segurança.
...
- Vítima vulnerável: `http://api.lab.localhost:8034` (bindada em `127.0.0.1:8034`)
- Vítima corrigida: `http://api.lab.localhost:8134` (bindada em `127.0.0.1:8134`)
- Página do atacante: `http://evil.lab.localhost:8234` (bindada em `127.0.0.1:8234`)

Use os hostnames `*.lab.localhost` no navegador (não as portas cruas em `127.0.0.1`) — os hostnames distintos são o que torna o atacante um irmão cross-origin da vítima. Pare com `./atom down cors-wildcard`. Se preferir Docker cru: `cd atoms/A05-security-misconfiguration/cors-wildcard && docker compose up --build`.
...
2. [`DIFF.md`](./DIFF.md) — o diff de uma-política entre `vulnerable/` e `fixed/`, e por que "confiar na nossa própria família de subdomínios", "só tirar as credenciais" e "usar `*`" não são o fix.

## Versão corrigida

A API corrigida na porta 8134 troca a reflexão da origem por uma **allowlist de correspondência exata**: ela ecoa `Access-Control-Allow-Origin` só quando a `Origin` do request é exatamente uma das origens em que confia (`partner.lab.localhost`, um irmão legítimo — aqui um stand-in). Uma origem não-confiável — o irmão hostil `evil.lab.localhost` — não recebe cabeçalho CORS nenhum, então o navegador bloqueia a leitura cross-origin e o script do atacante vê um erro de CORS, não o dado. A lição fica mais afiada aqui: **nem todo subdomínio irmão é confiável — você nomeia os exatos, não confia na família inteira.** `GET /account` continua devolvendo o mesmo dado à vítima logada, e a requisição ainda chega ao servidor com o cookie anexado — o navegador só se recusa a entregar a resposta a um script de uma origem que o servidor não autorizou. Veja o [`DIFF.md`](./DIFF.md) pra entender por que o fix é uma allowlist, e não refletir uma origem "específica", tirar as credenciais, ou o wildcard.
